...
class CustomMessageBox(tk.Toplevel):
    def __init__(self, parent, title, message):
        ...
        tk.Label(self, text=message, bg='#f0e6f7', font=('Comic Sans MS', 12)).pack(pady=20)

        button_frame = tk.Frame(self, bg='#f0e6f7')
        button_frame.pack(pady=10)

        ttk.Button(button_frame, text="OK", command=self.destroy, style='TButton').pack()
...
class FurnitureApp:
    def __init__(self, root):
        ...
        self.style.configure('TMenubutton', background='#ffe4e1', foreground='black', font=('Comic Sans MS', 12), padding=5)
        self.style.map('TMenubutton', background=[('active', '#ffd3b6')], relief=[('pressed', 'sunken'), ('!pressed', 'raised')])
        self.style.configure('TButton', background='#ffe4e1', foreground='black', font=('Comic Sans MS', 12), relief='raised')
        self.style.map('TButton', background=[('active', '#ffd3b6'), ('!active', '#ffe4e1')])

        self.leg_color_var = tk.StringVar(value="Black")  # leg color 기본값 설정
        self.leg_color_menu = None  # leg color OptionMenu 초기화

        # 프레임과 다른 위젯에도 같은 배경색 적용
    ...
    def create_widgets(self):
        # 모든 라벨과 버튼에 배경색 적용
        tk.Label(self.frame, text="Furniture Type:", bg='#f0e6f7', font=('Comic Sans MS', 12)).grid(row=0, column=0, pady=5, sticky='w')
        tk.Label(self.frame, text="Color:", bg='#f0e6f7', font=('Comic Sans MS', 12)).grid(row=1, column=0, pady=5, sticky='w')
        tk.Label(self.frame, text="Options:", bg='#f0e6f7', font=('Comic Sans MS', 12)).grid(row=3, column=0, pady=5, sticky='w')
        self.create_labels()
        self.create_options()
    ...
    def create_options(self):
        ...
        self.furniture_menu = tk.OptionMenu(self.frame, self.furniture_var, "Chair", "Wardrobe", "Desk", command=self.update_color_options)
        ...
        self.color_menu = tk.OptionMenu(self.frame, self.color_var, "White", "Grey", "Green")
        ...
        # leg_color_var 초기화
        self.leg_color_var = tk.StringVar(value="Black")  # 기본값 설정
    ...
    def update_color_options(self, *args):
        ...
        for color in colors:
            self.color_menu["menu"].add_command(label=color, command=tk._setit(self.color_var, color))
    ...
    def create_leg_color_menu(self):
        if self.leg_color_menu is None:
            self.leg_color_label = tk.Label(self.frame, text="Leg Color:", bg='#f0e6f7', font=('Comic Sans MS', 12))
            self.leg_color_label.grid(row=2, column=0, pady=5, sticky='w')
            self.leg_color_menu = tk.OptionMenu(self.frame, self.leg_color_var, "Black", "White")
            self.leg_color_menu.config(bg='#ffe4e1', fg='black', font=('Comic Sans MS', 12))
            self.leg_color_menu["menu"].config(bg='#ffe4e1', fg='black', font=('Comic Sans MS', 12))
            self.leg_color_menu.grid(row=2, column=1, pady=5)
        else:
            self.leg_color_menu["menu"].delete(0, "end")
            for color in ["Black", "White"]:
                self.leg_color_menu["menu"].add_command(label=color, command=tk._setit(self.leg_color_var, color))

    def create_checkbuttons(self):
        self.option_var = tk.BooleanVar()
        tk.Checkbutton(self.frame, text="Add Options", variable=self.option_var, command=self.toggle_options, bg='#f0e6f7', font=('Comic Sans MS', 12), selectcolor="#ffd3b6").grid(row=3, column=1, pady=5, sticky='w')

        self.option_frame = tk.Frame(self.frame, bg='#f0e6f7')
        self.option_frame.grid(row=4, columnspan=2, pady=5, sticky='w')

    def create_buttons(self):
        self.button_frame = tk.Frame(self.frame, bg='white', relief='raised', borderwidth=1)
        self.button_frame.grid(row=5, columnspan=2, pady=10, sticky='ew')

        create_button = ttk.Button(self.button_frame, text="🛠 Create Furniture", command=self.create_furniture, style='TButton')
        create_button.pack(side='left', padx=5)

        undo_button = ttk.Button(self.button_frame, text="↩ Undo", command=self.undo_furniture, style='TButton')
        undo_button.pack(side='left', padx=5)

        show_button = ttk.Button(self.button_frame, text="👁 Show Furniture", command=self.show_furniture, style='TButton')
        show_button.pack(side='left', padx=5)

    def create_result_box(self):
        self.result_text = tk.Text(self.frame, height=5, width=50, wrap="word", bg="#f0e6f7", fg="black", font=("Comic Sans MS", 12))
    ...
    def create_image_box(self):
        ...
        self.image_label.grid(row=7, column=0, columnspan=2, sticky="nsew", padx=20, pady=0)
        ...
        if self.option_var.get():
            ...
            if furniture == 'Chair':
                ...
                for option in options:
                    ...
                    check = tk.Checkbutton(frame, text=option, variable=var, bg='#f0e6f7', font=('Comic Sans MS', 12))
                    check.pack(side='left')
                    color_var = tk.StringVar(value="Black")
                    color_menu = tk.OptionMenu(frame, color_var, "Black", "White")
                    color_menu.config(bg='#ffe4e1', fg='black', font=('Comic Sans MS', 12))
                    color_menu["menu"].config(bg='#ffe4e1', fg='black', font=('Comic Sans MS', 12))
                ...
                for option in options:
                    var = tk.BooleanVar()
                    self.option_vars.append((option, var))
                    tk.Checkbutton(self.option_frame, text=option, variable=var, bg='#f0e6f7', font=('Comic Sans MS', 12)).pack(anchor='w')
            elif furniture == 'Desk':
    ...
    def create_furniture(self):
        ...
        if self.option_var.get():
            for option, var in self.option_vars:
                if var.get():
                    if option == "Wheel":
                        decorated_furniture = WheelDecorator(decorated_furniture, self.leg_color_var.get()) 
                    elif option == "Armrest":
                        decorated_furniture = ArmrestDecorator(decorated_furniture, color)
                    elif option == "Mirror":
    ...
    def show_furniture(self):
        ...
        try:
            ...
            tk.messagebox.showerror("Error", f"Image file not found: {filename}")
